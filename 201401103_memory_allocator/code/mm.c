...
#define PUT(p, val)  (*(uintptr_t *)(p) = (val) | GET_TAG(p))		//changed from (*(uintptr_t *)(p) = (val))
...
/* Some extra Macros */
#define LISTS 16	//defines number of lists we are making
...
//	size_t oldsize;
	size_t newsize = size;
	void *newptr = ptr;
	int sizeDiff, extendSize, block_buffer;
    ...
    block_buffer = GET_SIZE(HDRP(ptr)) - newsize;
    
    /* Allocate more space if overhead falls below the minimum */
    if (block_buffer < 0) {
        /* Check if next block is a free block or the epilogue block */
        if (!GET_ALLOC(HDRP(NEXT_BLKP(ptr))) || !GET_SIZE(HDRP(NEXT_BLKP(ptr)))) {
            sizeDiff = GET_SIZE(HDRP(ptr)) + GET_SIZE(HDRP(NEXT_BLKP(ptr))) - newsize;
            if (sizeDiff < 0) {
                extendSize = MAX(-sizeDiff, CHUNKSIZE);
            ...
            // Do not split block
            PUT(HDRP(ptr), PACK(newsize + sizeDiff, 1)); 
            PUT(FTRP(ptr), PACK(newsize + sizeDiff, 1)); 
        } else {
            newptr = mm_malloc(newsize - DSIZE);
            memcpy(newptr, ptr, MIN(size, newsize));
            mm_free(ptr);
        }
        block_buffer = GET_SIZE(HDRP(newptr)) - newsize;
    }
