...
/* Some extra Macros */
#define LISTS 20	//defines number of lists we are making
...
//#define ASSIGN_PTR(ptr, assignTo)  ( *(void **) ptr = assignTo )


/* Global variables: */
static char *heap_listp; /* Pointer to first block */  
//static char *last_listp; /* Pointer to block which was searched last */		// these are char* pointers so as to work with WSIZE and DSIZE
void *seg_free_list[LISTS];	/* array of pointers to all the linked lists */
...
//	reqdList = seg_free_list[list];
	...
	if(PREV_FREE_BLK(bp) != NULL) {
		...
		if(NEXT_FREE_BLK(bp) != NULL) {
			...
			seg_free_list[list] = NEXT_FREE_BLK(bp);
		}
		else {
			seg_free_list[list] = NULL;
		}
